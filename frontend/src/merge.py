...
with open('/Users/ramanraj/Documents/LockedIn/frontend/src/pages/Leaderboard.jsx', 'r') as f:
    code = f.read()

# Extract from start up to 'const Leaderboard = () => {'
header_part = code.split('const Leaderboard = () => {')[0]

# Remove SWIPER imports section so we can grab the rest cleanly
# It starts at // 🔥 SWIPER.JS IMPORTS
if '// 🔥 SWIPER.JS IMPORTS' in header_part:
    header_part = header_part.split('// 🔥 SWIPER.JS IMPORTS', 1)[1]
...
# Extract CSS and UI
ui_part = code.split('return (')[1]
ui_part = ui_part.rsplit(');', 1)[0]
# Remove Sidebar
...
new_file = f"""import React, {{ memo, useState, useEffect, useMemo }} from 'react';
...
const LiveFocusSection = () => {{
  ...
  return (
    <section className="relative w-full py-32 bg-[#FAF9F6] border-t border-[#E5E3DB]">
...
with open('/Users/ramanraj/Documents/LockedIn/frontend/src/components/showcase/LiveFocusSection.jsx', 'w') as f:
    f.write(new_file)
